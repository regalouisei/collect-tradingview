"""Python backtesting engine with Python 3.12 compatibility.
...
Usage:
    ...
    python framework/backtest_runner.py --limit 10
"""
...
try:
    ...
    raise RuntimeError(f"Required package not installed. Run: pip install {e.name}")
...
def load_strategy_from_file_v2(strategy_file: Path) -> type[Strategy]:
    """Load Strategy class from file with Python 3.12 compatibility.
    ...
    try:
        ...
        import importlib
        spec = importlib.util.spec_from_loader_location(strategy_file.name, strategy_file)
...
def run_single_ticker_backtest(strategy_file: Path, ticker: str) -> dict:
    ...
    try:
        # Load strategy
        strategy_class = load_strategy_from_file_v2(strategy_file)
        ...
        num_trades = len(stats['# Trades']) - 1 if '# Trades' in stats else 0
        win_rate_pct = (num_trades / (num_trades + 1)) * 100 if num_trades > 0 else 0
...
def main():
    ...
    print("=" * 60)
    print("Python 3.12 Compatible Backtest Runner")
    print("=" * 60)
    print(f"\nTesting on tickers: {list(TICKERS.values())}")
    ...
    if not strategy_files:
        print("No strategy files to backtest!")
        return
    ...
    print("=" * 60)
    print(f"\nTotal results: {len(total_results)}")
    print(f"Saved to: {RESULTS_FILE}")

    successful = sum(1 for r in total_results if not r['error'])
    print(f"Successful: {successful}/{len(total_results)}")
    print(f"Failed: {len(total_results) - successful}")
