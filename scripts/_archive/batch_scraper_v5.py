"""Batch scraper v5 - Page-based scraping (CORRECT VERSION).

Features:
- Page-based scraping (not scroll-based)
- Detects total pages dynamically
- Scrapes all pages for each category
- Enhanced error handling

Usage:
    python scripts/batch_scraper_v5.py --all
    python scripts/batch_scraper_v5.py --category oscillators --pages 1-42
"""
...
STATE_FILE = PROJECT_ROOT / "results" / ".scrape_state_v5.json"
HASHES_FILE = PROJECT_ROOT / "results" / ".script_hashes_v5.json"
PROGRESS_FILE = PROJECT_ROOT / "results" / ".scrape_progress_v5.json"
...
PRIORITY_CATEGORIES = {
    ...
    "low": ["volume", "moving_averagers", "volatility"],
}
...
def detect_total_pages(page) -> int:
    ...
    try:
        ...
        if has_next:
            print("  Has pagination, but can't detect total. Using default 50 pages.")
            return 50
...
def scrape_category(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Visit first page to detect total pages
            ...
            for page_num in range(1, total_pages + 1):
                ...
                for i, script in enumerate(scripts, 1):
                    ...
                    if i % 5 == 0 or i == len(scripts):
                        print(f"    [{i}/{len(scripts)}] {name[:60]}")
...
def scrape_categories(
    ...
    print("=" * 60)
    print("Batch Scraper v5 - Page-Based Scraping")
    print("=" * 60)
    print(f"\nConfiguration:")
...
def main():
    parser = argparse.ArgumentParser(description="Batch scraper v5 - Page-based scraping")
