#!/bin/bash
# Cline-assisted Pine Script conversion script (MiniMax M2.5)
# 自动运行，无需用户输入
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Cline 自动转换脚本 (MiniMax M2.5)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
for i in "${!PENDING_SCRIPTS[@]}"; do
    ...
    # 构建 Cline 提示词（简化版，避免超长）
    cline_prompt="Convert this Pine Script to Python trading class using backtesting.py.

Script: ${script_name}
Category: ${category}

Requirements:
1. Import: from backtesting import Strategy
2. Use pandas_ta for indicators
3. Class name: TvStrategy
...
Return ONLY Python code."
    ...
    # 使用 Cline 转换（MiniMax M2.5 - 免费）
    if cline --model minimax/minimax-m2-5 -t 300 -p /tmp/temp_pine.pine "$cline_prompt" > /tmp/temp_python.py 2>&1; then
        # 转换成功
...
echo ""
echo -e "${GREEN}💰 成本: $0.00 (MiniMax M2.5 免费)${NC}"
echo ""
