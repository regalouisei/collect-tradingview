"""Optimized batch scraper: Extract Pine Script source from TradingView using Playwright.

Features:
- Content-based deduplication (SHA256 hash)
- Multi-category concurrent scraping
- Aggressive scrolling for faster collection
- State persistence for incremental scraping

Usage:
    python scripts/batch_scraper_optimized.py --all --limit 200
    python scripts/batch_scraper_optimized.py --category editors_picks --limit 50
"""
...
def collect_script_urls(page, category: str, limit: int = 0, aggressive: bool = False) -> list[dict]:
    """Scroll through a category page and collect all script URLs (optimized)."""
    ...
    for i in range(max_scrolls):
        ...
        time.sleep(0.5 if aggressive else 1)  # Faster scrolling
...
def extract_pine_source(page, script_url: str, max_retries: int = 2) -> str | None:
    """Navigate to a script page and extract Pine Script source code (optimized)."""
...
def scrape_category(
    ...
    """Scrape a single category (optimized)."""
    print(f"\n{'='*60}")
    ...
    with sync_playwright() as p:
        ...
        try:
            # Collect script URLs
            scripts = collect_script_urls(page, category, limit, aggressive)
...
def scrape_categories(
    ...
    """Scrape multiple categories concurrently (optimized)."""
    print("=" * 60)
    print("Optimized Batch Scraper - Concurrent Scraping")
    print("=" * 60)
    print(f"\nConfiguration:")
...
def main():
    parser = argparse.ArgumentParser(description="Optimized batch scraper for TradingView")
