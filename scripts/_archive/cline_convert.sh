#!/bin/bash
# Cline-assisted Pine Script conversion script
...
# 询问是否继续
echo -e "${CYAN}[3/3] 使用 Cline 转换${NC}"
echo -e "  即将使用 Cline 转换 ${#PENDING_SCRIPTS[@]} 个脚本"
echo -e "  这可能需要较长时间..."
echo ""
...
Trading rules:
- Create reasonable entry/exit rules based on the indicator logic
...
\`\`\`"
    ...
    # 使用 Cline 转换
    if cline -m claude-sonnet-4-5-20241022 -t 300 -p /tmp/temp_pine.pine "$cline_prompt" > /tmp/temp_python.py 2>&1; then
        # 转换成功
...
echo ""
echo -e "${GREEN}💰 成本: $0.00 (Cline 使用其他模型)${NC}"
echo ""
