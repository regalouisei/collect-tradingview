#!/bin/bash
# Cline-assisted Pine Script conversion script (AUTO MODE - FIXED)
# 自动运行，无需用户输入
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Cline 自动转换脚本（修复版）${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
for i in "${!PENDING_SCRIPTS[@]}"; do
    ...
    # 构建 Cline 提示词（简化版，避免模型错误）
    cline_prompt="Convert this Pine Script to a Python trading strategy class.
...
\`\`\`"
    ...
    # 使用 Cline 转换（使用正确的 Claude Sonnet 模型）
    if cline -m anthropic/claude-sonnet-4-20250514 -t 300 -p /tmp/temp_pine.pine "$cline_prompt" > /tmp/temp_python.py 2>&1; then
        # 转换成功
...
echo ""
echo -e "${GREEN}💰 成本: 使用你的 Cline 账户${NC}"
echo ""
