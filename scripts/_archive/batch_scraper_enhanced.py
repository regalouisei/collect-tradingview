"""Optimized batch scraper - ENHANCED VERSION.
...
Usage:
    python scripts/batch_scraper_enhanced.py --all --limit 200 --aggressive --workers 8
    python scripts/batch_scraper_enhanced.py --category editors_picks --limit 100
"""
...
STATE_FILE = PROJECT_ROOT / "results" / ".scrape_state_enhanced.json"
...
def collect_script_urls(page, category: str, limit: int = 0, aggressive: bool = False, 
    ...
    max_scrolls = 200 if aggressive else 120  # ⬆️ Increased
    no_new_threshold = 7 if aggressive else 5  # ⬆️ Increased
    scroll_delay = 1 if aggressive else 2  # ⬆️ Optimized
...
def scrape_categories(
    ...
    total_saved = sum(r.get("saved", 0) for r in all_results.values())
    total_skipped = sum(r.get("skipped", 0) for r in all_results.values())
    total_duplicate = sum(r.get("duplicate", 0) for r in all_results.values())
    total_closed = sum(r.get("closed", 0) for r in all_results.values())
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced batch scraper for TradingView")
    ...
    parser.add_argument("--max-scrolls", type=int, default=200, help="Max scroll attempts")
    parser.add_argument("--scroll-stop-threshold", type=int, default=7, help="Consecutive no-new-content scrolls before stopping")
    parser.add_argument("--scroll-delay", type=int, default=1, help="Seconds between scrolls")
    parser.add_argument("--random-delay", type=int, default=2, help="Random delay range (1-N) seconds")
    ...
    results = scrape_categories(
        ...
        random_delay=(1, args.random_delay),
    )
